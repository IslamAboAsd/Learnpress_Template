...
$heading       = sprintf( esc_html__( 'Other results (newest %d items)', 'kipso' ), $limit );
$heading       = apply_filters( 'learn_press_quiz_history_heading', $heading );
?>

<?php if ( $heading ) { ?>
    <h4 class="lp-group-heading-title toggle-off"
        onclick="LP.toggleGroupSection('#lp-quiz-history', this);"><?php echo esc_html($heading); ?>
...
<?php if ( $history_count > 1 ) { ?>
    ...
    <div class="lp-group-content-wrap hide-if-js" id="lp-quiz-history">
        <table class="quiz-history">
            ...
            <tr>
                ...
                <th><?php esc_html__( 'Time', 'kipso' ); ?></th>
                <th><?php esc_html__( 'Result', 'kipso' ); ?></th>
            </tr>
            </thead>
			<?php foreach ( $history as $item ) {
				...
				$position ++; ?>
                <tr>
                    <td align="right"><?php echo esc_html($position); ?></td>
                    <td>
	...
	learn_press_display_message( esc_html__( 'No history found!', 'kipso' ) );
}