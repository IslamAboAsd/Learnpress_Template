...
<?php if ( $prev_id  ) { ?>
    ...
    <form name="prev-question" class="prev-question form-button lp-form" method="post"
          action="<?php echo esc_attr($quiz->get_question_link( $prev_id )); ?>">

		<?php do_action( 'learn-press/quiz/begin-prev-question-button' ); ?>

        <button type="submit"><?php echo esc_html_x( 'Prev', 'quiz-question-navigation', 'kipso' ); ?></button>
        <input type="hidden" name="question-id" value="<?php echo esc_attr($current_question_id); ?>">
...
<?php if ( $next_id ) { ?>
    ...
    <form name="next-question" class="next-question form-button lp-form" method="post"
          action="<?php echo esc_attr($quiz->get_question_link( $next_id )); ?>">

		<?php do_action( 'learn-press/quiz/begin-next-question-button' ); ?>

        <button type="submit"><?php echo esc_html_x( 'Next', 'quiz-question-navigation', 'kipso' ); ?></button>
        <input type="hidden" name="question-id" value="<?php echo esc_attr($current_question_id); ?>">
...
<?php if ( $next_id  && ! $user->has_completed_quiz( $quiz->get_id(), $course_id ) ) { ?>
    ...
    <form name="skip-question" class="skip-question form-button lp-form" method="post"
        ...
        <button type="submit"><?php echo esc_html_x( 'Skip', 'quiz-question-navigation', 'kipso' ); ?></button>
        <input type="hidden" name="question-id" value="<?php echo esc_attr($current_question_id); ?>">
