...
$button_text = $hinted ? esc_html__( 'Hinted', 'kipso' ) : esc_html__( 'Hint', 'kipso' );
?>
...
<?php do_action( 'learn-press/quiz/before-question-hint-button' ); ?>
    ...
    <form name="question-hint" class="question-hint form-button lp-form" method="post" enctype="multipart/form-data">
		...
		<?php if ( 0 > $quiz->get_show_hint() ) { ?>
            ...
            <button type="submit" <?php disabled( $hinted ); ?>><?php echo esc_html($button_text); ?></button>

		<?php } else { ?>

            <button class="lp-button button-hint-question"
                    type="submit"
                    data-counter="<?php echo esc_attr($user->can_hint_answer( $quiz->get_id() )); ?>"
				<?php disabled( $hinted ); ?>><?php echo esc_html($button_text); ?></button>
        ...
        <input type="hidden" name="question-id" value="<?php echo esc_attr($question->get_id()); ?>">
