...
$button_text = $checked ? esc_html__( 'Checked', 'kipso' ) : esc_html__( 'Check', 'kipso' );
?>
...
<?php do_action( 'learn-press/quiz/before-check-answer-button' ); ?>
    ...
    <form name="check-answer-question" class="check-answer-question form-button lp-form" method="post"
		...
		<?php if ( 0 > $quiz->get_show_check_answer() ) { ?>
            ...
            <button type="submit" <?php disabled( $checked ); ?>><?php echo esc_html($button_text); ?></button>

		<?php } else { ?>

            <button type="submit"
                    class="button-check-answer"
                    data-counter="<?php echo esc_attr($user->can_check_answer( $quiz->get_id() )); ?>"
