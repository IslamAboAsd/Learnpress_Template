...
<div class="course-rating">
	<h3><?php esc_html_e( 'Reviews', 'learnpress' ); ?></h3>
	<div class="average-rating" itemprop="aggregateRating" itemscope=""
		 itemtype="http://schema.org/AggregateRating">
		<p class="rating-title"><?php esc_html_e( 'Average Rating', 'learnpress' ); ?></p>
		...
		<div class="rating-box">
			...
			<div class="review-amount" itemprop="ratingCount">
				<?php $total ? printf( _n( '%1$s rating', '%1$s ratings', $total, 'learnpress' ), number_format_i18n( $total ) ) : esc_html_e( '0 rating', 'learnpress' ); ?>
			</div>
		</div>
	</div>
	<div class="detailed-rating">
		<p class="rating-title"><?php esc_html_e( 'Detailed Rating', 'learnpress' ); ?></p>
				...
				if ( isset( $course_rate_res['items'] ) && !empty( $course_rate_res['items'] ) ):
					foreach ( $course_rate_res['items'] as $item ):
						...
						<div class="stars">
							<div class="key">
								<?php if ( $item['rated'] == '1' ) {
									esc_html_e( $item['rated'] ); ?><?php esc_attr__( ' Star', 'learnpress' );
								} else {
									esc_html_e( $item['rated'] ); ?><?php esc_attr__( ' Stars', 'learnpress' );
								} ?>
							</div>
							<div class="bar">
								<div class="full_bar">
