...
<?php if ( $items = $section->get_items() ) { ?>
    ...
    <ul class="section-content">
		...
		<?php foreach ( $items as $item ) { ?>
            <?php 
               $lesson_format = 'standard'; 
               if( get_post_format($item->get_id()) ){
                  $lesson_format = get_post_format($item->get_id());
               }
            ?> 
            <li class="lesson-format-<?php echo esc_attr($lesson_format); ?> <?php echo join( ' ', $item->get_class() ); ?>">
				<?php
				if ( $item->is_visible() ) {
					/**
					 * @since 3.0.0
...
<?php } else { ?>
	...
	<?php learn_press_display_message( esc_html__( 'No items in this section', 'kipso' ) ); ?>
